...
function parse(re, _debug) {
  ...
  if (valid) {
    const groupCount = stack.groupCounter ? stack.groupCounter.i : 0;
...
function _fixNodes(stack, re, endIndex) {
  ...
  stack.reduce((endIndex, node) => {
    ...
    if (node.type === GROUP_NODE || (node.type === ASSERT_NODE && node.sub)) {
      ...
      node.branches.reduce((endIndex, branch) => {
        _fixNodes(branch, re, endIndex);
        const head = branch[0]; // Reversed,so branch[0] is head.Dammit mystic code
...
var actions = (function _() {
  ...
  function endChoice(stack) {
    if (stack._parentChoice) {
      const choice = stack._parentChoice;
      delete stack._parentChoice;
      delete stack._parentGroup;
      delete stack.groupCounter;
      const parentStack = choice._parentStack;
  ...
  function charsetRangeEnd(stack, c, i, state, s) {
    const charset = stack[0];
    let range = charset.chars.slice(-2);
    range = [range[0], c];
    range.lastIndex = i;
  ...
  function charsetRangeEndHexEscape(stack, c, i) {
    const charset = stack[0];
    const code = charset.chars.slice(-1) + c;
  ...
  */
  ...
  return {
    ...
    charsetClassEscape,
    charsetHexEscape,
    charsetUnicodeEscape,
    charsetRangeEnd,
    charsetNormalEscape,
    charsetRangeEndNormalEscape,
...
const exactEXCharset = '^+*?^$.|(){[\\';

const charClassEscape = 'dDwWsS';
const unicodeEscape = 'u';
const hexEscape = 'x';
// var octDigit='0-7';
...
const normalEscapeInCharsetEX = `^${charClassEscape}${unicodeEscape}${hexEscape}0-9`;

// 'rntvf\\' escape ,others return raw
// Also need exclude \b\B assertion and backref
const normalEscapeEX = `${normalEscapeInCharsetEX}bB1-9`;

// var controlEscape;//Never TODO.Same reason as OctEscape.

const repeatnStates = 'repeatnStart,repeatn_1,repeatn_2,repeatnErrorStart,repeatnError_1,repeatnError_2';
const hexEscapeStates = 'hexEscape1,hexEscape2';
const unicodeEscapeStates = 'unicodeEscape1,unicodeEscape2,unicodeEscape3,unicodeEscape4';
