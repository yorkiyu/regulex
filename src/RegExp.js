...
function RegExp(re, options) {
  ...
  ast.traverse((node) => { explainCharset(node, ignoreCase); }, CHARSET_NODE);
  ast.traverse((node) => { explainExact(node, ignoreCase); }, EXACT_NODE);
...
RegExp.prototype = {
  ...
  exec(s) {
    const nfa = this.getNFA(); let ret;
    let startIndex = this.global ? (this.lastIndex || 0) : 0; const max = s.length;
...
// Not used
const charClass2ranges = { //  e.g. \d\D\w\W\s\S
  d: ['09'],
  w: ['AZ', 'az', '09', '_'],
  s: ' \f\n\r\t\v\u1680\u180e\u2000\u2001\u2002\u2003\u2004\u2005\u2006\u2007\u2008\u2009\u200a\u2028\u2029\u202f\u205f\u3000'.split(''),
};
['d', 'w', 's'].forEach((cls) => { // D W S,negate ranges
...
function expandRangeIgnoreCase(ranges) {
  return K.flatten2(ranges.map((r) => {
    const parts = K.classify([r, 'az', 'AZ']).map[r];
    return K.flatten2(parts.map((p) => {
      if (/[a-z]/.test(p)) {
...
var NFABuilders = (function _() {
  function exact(node, from) {
    const ts = []; let to; const ranges = node.explained;
  ...
  function backref(node, from) {
    const to = [newState()]; const groupNum = node.num;
    return {
      accepts: to,
      trans: [{
        from,
...
function repeatNFA(node, from) {
  ...
  let a; let i; let trans = []; const { repeat } = node;
  const { min } = repeat; let { max } = repeat;
  i = min;
  while (i--) {
    a = builder(node, from);
    trans = trans.concat(a.trans);
